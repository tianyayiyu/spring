...
class ConfigurationClassEnhancer {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		//判断是否被代理过，被代理过的配置类会实现EnhancedConfiguration这个接口
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		//增强接口，实现EnhancedConfiguration接口，EnhancedConfiguration实现了 BeanFactoryAware接口
		//BeanFactoryAware有方法获取代理类的可用的对象 bean
		enhancer.setInterfaces(new Class<?>[] {EnhancedConfiguration.class});
		enhancer.setUseFactory(false);
		enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
		//
		/**
		 * BeanFactoryAwareGeneratorStrategy是cglib生成累的一个策略
		 * 主要为生成cglib的代理类中添加成员变量$$beanFactory，同时基于接口
		 * EnhancedConfiguration的父接口BeanFactoryAware中的setBeanFactory方法
		 * 设置此变量的值为当前Context中的BeanFactory，，因此我们这个cglib代理类就有了
		 * beanFactory，有了factory就能获得对象，不用通过方法获得对象了，
		 * 该BeanFactory的作用就是this调用时拦截该调用，并直接在beanFactory中获得目标bean
		 */
...
//enhancedConfigInstance代理
			//通过enhancedConfigInstance中的成员变量$$BeanFactory获得beanFactory
...
//判断执行的方法和调用方法是不是同一个方法
			 //其实就是判断是new还是getBean （new表示第一次执行，getbean表示已经new过了，用getabean获取）
